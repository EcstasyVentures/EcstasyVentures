...
app.get("/api/founders", async (req, res) => {
    try {
        const founders = await FounderAuth.find().sort({ createdAt: -1 });
        res.json(founders);
    } catch (e) {
...
app.post("/api/founders", async (req, res) => {
    try {
        const { email, password, name, founderId, company, stage } = req.body;
        if (!email || !password || !name || !founderId) return res.status(400).json({ error: "Missing required fields" });
        const exists = await FounderAuth.findOne({ email });
        if (exists) return res.status(409).json({ error: "Founder already exists" });
        const hashed = await bcrypt.hash(password, 10);
        const founder = await FounderAuth.create({ email, password: hashed, name, founderId, company, stage });
        res.json(founder);
    } catch (e) {
        res.status(500).json({ error: "Server error" });
    }
...
// Update founder
app.put("/api/founders/:founderId", async (req, res) => {
    try {
        const { founderId } = req.params;
        const update = { ...req.body };
        if (update.password) update.password = await bcrypt.hash(update.password, 10);
        const founder = await FounderAuth.findOneAndUpdate({ founderId }, update, { new: true });
...
// Delete founder
app.delete("/api/founders/:founderId", async (req, res) => {
    try {
        const { founderId } = req.params;
        const founder = await FounderAuth.findOneAndDelete({ founderId });
        if (!founder) return res.status(404).json({ error: "Founder not found" });
...
const NotificationSchema = new mongoose.Schema({
    ...
    adminId: { type: String, default: 'all' }
});
...
async function createTestAdmins() {
    try {
        const adminUsers = [
            { username: "superadmin", password: await bcrypt.hash("super123", 10), role: "superAdmin", permissions: ["dashboard", "ventures", "founders", "deals", "tasks", "growth", "finance", "legal", "documents", "crm", "support", "reports", "automations", "users", "settings"], loginCount: 0 },
            { username: "operations", password: await bcrypt.hash("ops123", 10), role: "operationsAdmin", permissions: ["dashboard", "ventures", "founders", "tasks", "support", "reports"], loginCount: 0 },
            { username: "venture", password: await bcrypt.hash("venture123", 10), role: "venturePartner", permissions: ["dashboard", "ventures", "deals", "crm", "reports"], loginCount: 0 },
            { username: "finance", password: await bcrypt.hash("finance123", 10), role: "financeAdmin", permissions: ["dashboard", "finance", "documents", "reports"], loginCount: 0 },
            { username: "legal", password: await bcrypt.hash("legal123", 10), role: "legalAdmin", permissions: ["dashboard", "legal", "documents", "reports"], loginCount: 0 },
            { username: "marketing", password: await bcrypt.hash("market123", 10), role: "growthMarketing", permissions: ["dashboard", "growth", "reports"], loginCount: 0 },
            { username: "techlead", password: await bcrypt.hash("tech123", 10), role: "techLead", permissions: ["dashboard", "tasks", "automations", "reports"], loginCount: 0 }
        ];
...
app.post("/api/contact", async (req, res) => {
    try {
        ...
        const newContact = new Contact({ name, email, startup: startup || "", message });
        await newContact.save();
        res.status(201).json({ success: true, message: "Contact form submitted successfully", contactId: newContact._id });
    } catch (err) {
...
app.get("/api/admin/login-stats", async (req, res) => {
    try {
        ...
        res.json({ totalLogins, users: admins.map(admin => ({ username: admin.username, role: admin.role, loginCount: admin.loginCount })) });
    } catch (err) {
...
app.get("/api/admin/login-history/:username", async (req, res) => {
    try {
        ...
        const history = await LoginHistory.find({ username }).sort({ loginTime: -1 }).limit(50);
        if (history.length === 0) return res.status(404).json({ message: "No login history found for this user" });
        res.json(history);
    } catch (err) {
...
app.put("/api/founders/:id", async (req, res) => {
    try {
        ...
        const founder = await Founder.findOneAndUpdate({ id }, updateData, { new: true });
        if (!founder) return res.status(404).json({ error: "Founder not found" });
...
app.put("/api/ventures/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const updateData = { ...req.body, updatedAt: new Date() };
        const venture = await Venture.findOneAndUpdate({ id }, updateData, { new: true });
...
app.put("/api/tasks/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const updateData = { ...req.body, updatedAt: new Date() };
        const task = await Task.findOneAndUpdate({ id }, updateData, { new: true });
...
app.put("/api/documents/:id", async (req, res) => {
    try {
        const { id } = req.params;
        const updateData = { ...req.body, updatedAt: new Date() };
        const document = await DocumentModel.findOneAndUpdate({ id }, updateData, { new: true });
...
app.get("/api/search", async (req, res) => {
    try {
        ...
        const [founders, ventures, tasks] = await Promise.all([
            Founder.find({
                $or: [{ name: searchRegex }, {email: searchRegex }, { company: searchRegex }, { role: searchRegex }]
            }),
            Venture.find({
                $or: [{ name: searchRegex }, {description: searchRegex }, { industry: searchRegex }]
            }),
            Task.find({
                $or: [{ title: searchRegex }, {description: searchRegex }, { assignee: searchRegex }]
            })
        ]);

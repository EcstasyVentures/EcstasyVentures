...
// Define FounderAuth schema for founder authentication
...
// Define LoginHistory schema for tracking admin login timestamps
...
// Define UserData schema with flexible structure
...
// Function to create test founder users
async function createTestFounders() {
    try {
        const founders = [
            { 
                email: "founder@test.com", 
                password: await bcrypt.hash("founder123", 10), 
                name: "Test Founder", 
                founderId: "founder_test_001", 
                company: "Test Startup", 
                stage: "Seed",
                // Additional fields for Founder model
                phone: "",
                kyc: "Pending",
                role: "",
                experience: "",
            ...
            { 
                ...
                password: await bcrypt.hash("startup123", 10), 
                name: "John Doe", 
                founderId: "founder_john_002", 
        ...
        for (const f of founders) {
            // Check if FounderAuth exists
            const authExists = await FounderAuth.findOne({ email: f.email });
            if (!authExists) {
                await FounderAuth.create({
                    email: f.email,
                    password: f.password,
                    name: f.name,
                ...
                console.log(`✅ Created FounderAuth for ${f.email}`);
            }

            // Check if Founder exists
            const founderExists = await Founder.findOne({ id: f.founderId });
            if (!founderExists) {
                ...
                console.log(`✅ Created Founder for ${f.email}`);
            }
        }
    } catch (err) {
        console.error("Error creating test founders:", err);
    }
}

// Admin login route with login count and history tracking
...
// Get detailed login history for all admins
